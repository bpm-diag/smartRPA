...
def __handle_df(df1):
        ...
        programs_to_ignore = ["sppsvc.exe", "WMIC.exe", "git.exe", "BackgroundTransferHost.exe",
        ...
        appsMask = ~df1['application'].isin(programs_to_ignore)
        ...
        df1['browser_url_hostname'] = df1['browser_url'].apply(lambda url: getHostname(url)).fillna('')
        ...
        duplication_subset = ['category', 'application', 'concept:name', 'event_src_path', 'event_dest_path',
                                   'browser_url_hostname', 'xpath']

        # Use crosstab for get counts per ID and combinations (url_host, action)
        df_temp = pandas.crosstab(
            [df1[col] for col in duplication_subset],
...
def _process_info(v, df, df1, total_time, number_of_decision_points, SAVE_FILE):
    ...
    validation_folder = os.path.join(desktop, 'smartRPA_validation')
    __createDirectory(validation_folder)
...
"""
    ...
    if SAVE_FILE:
        csv_path = os.path.join(validation_folder, filename + '.csv')
        df.to_csv(csv_path)

        # with open(os.path.join(validation_folder, 'results_summary.txt'), 'a') as f:
        #     f.write(summary)
        ...
        fields = [
            ...
            " ".join(process_info.split()).replace('\n', '') + ' ' + success
        ]
        with open(os.path.join(validation_folder, 'results_summary.csv'), 'a', newline='\n') as f:
            writer = csv.writer(f)
...
'''
Questa funzione genera n tracce di validazione con i seguenti parametri di input:
- log_size: Dimensione del log (numero di tracce presenti in un log): 250/500/750/1000
- trace_size: Dimensione di una traccia (numero di eventi presenti nella traccia): 25/50/75/100
- events_size: Dimensione dell'alfabeto (eventi differenti): 40/80/120
- decision_point: numero di punti di decisione presenti: 1/2/3/4/5
'''
def validate_all():
    ...
    for log_size in [250, 500, 750, 1000]:
        for trace_size in [25, 50, 75, 100]:
            for events_size in [40, 80, 120]:
                for decision_point in [1, 2, 3, 4, 5]:
                    validate_helper(log_size, trace_size, events_size, decision_point)

    DEBUG_total_time = (time.time() - DEBUG_start_time)
    print(f"[DEBUG] Total execution time: {round(DEBUG_total_time, 3)} seconds\n")


def validate_helper(log_size, trace_size, events_size, decision_point, SAVE_FILE=True):
    while True:
        v = Validation(
            log_size=log_size,
        ...
        try:
            total_time, number_of_decision_points = _calculate_time_dp(df1)
        except ValueError as e:
            print(e)
            continue

        _process_info(v, df, df1, total_time,
...
if __name__ == '__main__':
    ...
    # validate_all() # generate n csv validation files with all parameters
    validate_helper(250, 25, 40, 1)  # TEST, generate 1 csv validation file with specified parameters
