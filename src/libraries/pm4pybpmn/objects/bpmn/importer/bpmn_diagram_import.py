...
from libraries.pm4pybpmn.objects.bpmn.importer import bpmn_python_consts as consts
...
class BpmnDiagramGraphImport(object):
    ...
    def import_sequence_flow_to_graph(diagram_graph, sequence_flows, process_id, flow_element):
        ...
        diagram_graph.edges[source_ref, target_ref][consts.Consts.id] = flow_id
        diagram_graph.edges[source_ref, target_ref][consts.Consts.process] = process_id
        diagram_graph.edges[source_ref, target_ref][consts.Consts.name] = name
        diagram_graph.edges[source_ref, target_ref][consts.Consts.source_ref] = source_ref
        diagram_graph.edges[source_ref, target_ref][consts.Consts.target_ref] = target_ref
        for element in utils.BpmnImportUtils.iterate_elements(flow_element):
            if element.nodeType != element.TEXT_NODE:
                ...
                if tag_name == consts.Consts.condition_expression:
                    ...
                    diagram_graph.edges[source_ref, target_ref][consts.Consts.condition_expression] = {
    ...
    def import_flow_di(diagram_graph, sequence_flows, message_flows, flow_element):
        ...
        if flow_data is not None:
            ...
            diagram_graph.edges[source_ref, target_ref][consts.Consts.waypoints] = waypoints
            diagram_graph.edges[source_ref, target_ref][consts.Consts.name] = name
