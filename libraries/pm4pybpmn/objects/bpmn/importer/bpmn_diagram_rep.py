...
class BpmnDiagramGraph(object):
    ...
    def get_nodes_list_by_process_id(self, process_id):
        ...
        nodes = []
        for node in tmp_nodes:
            if len(node) > 1 and consts.Consts.process in node[1] and node[1][consts.Consts.process] == process_id:
                nodes.append(node)
        return nodes
    ...
    def add_sequence_flow_to_diagram(self, process_id, source_ref_id, target_ref_id, sequence_flow_name=""):
        ...
        flow = self.diagram_graph.edges[source_ref_id, target_ref_id]
